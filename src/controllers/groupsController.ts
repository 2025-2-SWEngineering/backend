...
import { getFcmTokensByUserId } from "../models/fcmTokenModel.js";
import { unsubscribeTokensFromTopic } from "../services/fcmService.js";
...
export async function changeRole(
  ...
  try {
    ...
    if (!groupId || !targetUserId)
      return res.status(400).json({ message: "groupId, userId가 필요합니다." });
    if (!["admin", "member"].includes(roleStr))
      return res
        .status(400)
        .json({ message: "role은 admin|member 만 허용됩니다." });
    ...
    if (currentRole === "admin" && roleStr === "member") {
      ...
      if (adminCount <= 1)
        return res
          .status(400)
          .json({ message: "마지막 관리자는 강등할 수 없습니다." });
    }

    const updated = await setUserGroupRole({
      userId: targetUserId,
      groupId,
      role: roleStr as "admin" | "member",
    });
...
export async function remove(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (role !== "admin")
      return res
        .status(403)
        .json({ message: "그룹 삭제는 관리자만 가능합니다." });
    // Unsubscribe all member tokens from the group's topic before deletion
    try {
      const members = await listGroupMembers(groupId);
      for (const m of members) {
        try {
          const tokens = await getFcmTokensByUserId(m.user_id);
          if (tokens.length > 0)
            await unsubscribeTokensFromTopic(tokens, `group_${groupId}`);
        } catch (e) {
          // ignore per-user errors
          // eslint-disable-next-line no-console
          console.warn(
            "[FCM] failed unsubscribing tokens for member",
            m.user_id,
            e
          );
        }
      ...
      // ignore listing/unsubscribe errors
      // eslint-disable-next-line no-console
      console.warn(
        "[FCM] failed to unsubscribe tokens for group before deletion",
        e
      );
    }
    ...
    if (role === "admin") {
      ...
      if (adminCount <= 1 && memberCount > 1) {
        return res
          ...
          .json({
            message:
              "마지막 관리자는 다른 관리자를 지정하거나 그룹을 삭제해야 합니다.",
          });
      }
    }

    await removeUserFromGroup({ userId: req.user!.id, groupId });
    // unsubscribe user's tokens from group topic
    try {
      const tokens = await getFcmTokensByUserId(req.user!.id);
      if (tokens.length > 0)
        await unsubscribeTokensFromTopic(tokens, `group_${groupId}`);
    } catch (e) {
      // ignore errors
      // eslint-disable-next-line no-console
      console.warn("[FCM] failed to unsubscribe tokens from group topic", e);
    }
    return res.status(204).send();
  } catch (err) {
    next(err);
...
export async function removeMember(
  ...
  try {
    ...
    if (targetUserId === req.user!.id)
      return res
        .status(400)
        .json({ message: "본인은 추방할 수 없습니다. 탈퇴를 이용하세요." });
    ...
    if (targetRole === "admin") {
      ...
      if (adminCount <= 1)
        return res
          .status(400)
          .json({ message: "마지막 관리자는 추방할 수 없습니다." });
    }
    await removeUserFromGroup({ userId: targetUserId, groupId });
    // unsubscribe target user's tokens from group topic
    try {
      const tokens = await getFcmTokensByUserId(targetUserId);
      if (tokens.length > 0)
        await unsubscribeTokensFromTopic(tokens, `group_${groupId}`);
    } catch (e) {
      // ignore errors
